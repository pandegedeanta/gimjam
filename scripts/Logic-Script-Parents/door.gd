...
func eksekusi_pintu():
	...
	# === BUKA PINTU TANPA KUNCI ===
	is_open = true
	ui_node.show_message("Pintu Terbuka", Color.AQUAMARINE)

	# Ganti tampilan sprite (kalau pakai atlas)
	if sprite and koordinat_buka != Rect2():
		sprite.region_rect = koordinat_buka

	# Supaya tidak bisa diklik ulang
	input_pickable = false
	Input.set_default_cursor_shape(Input.CURSOR_ARROW)

func _on_body_entered(body):
	print("BODY MASUK:", body.name)
	print("is_open:", is_open, "transitioning:", transitioning)
